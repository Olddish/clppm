...
def collate_fn(datasetx):
    ...
    return micro_inputs, fea_inputs, torch.Tensor(labels), torch.tensor(complt_alloy_poss)
...
class Model(torch.nn.Module):
    ...
    def forward(self,mirco_inputs,fea_inputs,alloy_pos):
        ...
        combined = torch.cat((micro_with_alloy, fea_sentence_vec), dim=1)
...
def train_loop(model,train_loader,val_loader,epoch_num,lr):
    ...
    writer1 = SummaryWriter("task1-v3/information_fusion/text-text/logs/2f-no-pro/"+str(time)+'/train')
    writer2 = SummaryWriter("task1-v3/information_fusion/text-text/logs/2f-no-pro/"+str(time)+'/val')

    for epoch in tqdm(range(epoch_num),ncols=80):
        model.train()

        for step, (train_micro_inputs,train_fea_inputs,train_labels,train_alloy_poss) in enumerate(train_loader):
            ...
            with autocast():
                train_outputs = model(train_micro_inputs,train_fea_inputs,train_alloy_poss)
        ...
        model.float()
        model.eval()
        for step, (val_micro_inputs,val_fea_inputs,val_labels,val_alloy_poss) in enumerate(val_loader):
            ...
            with torch.no_grad():
                val_outputs = model(val_micro_inputs,val_fea_inputs,val_alloy_poss)
...
def test_loop(model,test_loader):
    ...
    model.to(device)
    model.eval()
    for step, (test_micro_inputs,test_fea_inputs,test_labels,test_alloy_poss) in enumerate(test_loader):
        ...
        with torch.no_grad():
            test_outputs = model(test_micro_inputs,test_fea_inputs,test_alloy_poss)
...
if __name__ == "__main__":
    ...
    df_val.to_excel('task1-v3/information_fusion/text-text/results/val/'+str(time)+"-2f-nopro.xlsx",  index=False)
    df_train = pd.DataFrame({'Train Outputs': train_outputs, 'Train Labels': train_labels})
    df_train.to_excel('task1-v3/information_fusion/text-text/results/train/'+str(time)+"-2f-nopro.xlsx", index=False)
    trained_model.float()
    #测试集数据加载
    ...
    df.to_excel('task1-v3/information_fusion/text-text/results/test/'+str(time)+"-2f-nopro.xlsx", index=False)
