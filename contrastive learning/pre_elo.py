...
class PEDataset(Dataset):
    def __init__(self, process_texts, fea_texts, labels, alloy_poss):
        self.labels = labels
    ...
    def __getitem__(self, index):
        ...
        return process_text, fea_text, label, alloy_pos
...
def tokenize_input(texts):
    model_name = 'task1-v3/contras_learning/encoder_v1/tokenizer'
...
def collate_fn(datasetx):
    ...
    fea_texts = [i[1] for i in datasetx]
    labels = [i[2] for i in datasetx]
    ...
    return process_inputs, fea_inputs, torch.Tensor(labels), torch.tensor(complt_alloy_poss)
...
def vec_pool(word_vec:torch.Tensor,pos:torch.Tensor):
    ...
    for batch_num in range(pos.size(0)):
        ...
        pos_maskt = pos[batch_num,:][pos[batch_num,:] != -1] # 去除包含-1的索引
...
class Model(torch.nn.Module):
    def __init__(self,model_name_phy,model_name_process):
        super(Model,self).__init__()
        
        self.fc1 = torch.nn.Linear(768*3, 768)
        self.bn1 = torch.nn.BatchNorm1d(768)
        self.dr1 = torch.nn.Dropout(0.5)
        self.fc2 = torch.nn.Linear(768,1)

        self.PhyEncoder = AutoModel.from_pretrained(model_name_phy)
        for param in self.PhyEncoder.parameters():
            param.requires_grad = False

        self.ProcessEncoder = AutoModel.from_pretrained(model_name_process)
        for param in self.ProcessEncoder.parameters():
            param.requires_grad = False


    def forward(self,process_inputs,fea_inputs,alloy_pos):

        process_vec = self.ProcessEncoder(**process_inputs).last_hidden_state
        fea_vec = self.PhyEncoder(**fea_inputs).last_hidden_state
        ...
        combined = torch.cat((process_with_alloy, fea_sentence_vec), dim=1)
...
def train_loop(model,train_loader,val_loader,epoch_num,lr):
    ...
    writer1 = SummaryWriter("task1-v3/contras_learning/logs/pre/"+str(time)+'/train')
    writer2 = SummaryWriter("task1-v3/contras_learning/logs/pre/"+str(time)+'/val')

    for epoch in tqdm(range(epoch_num),ncols=80):
        model.train()
        for step, (train_process_inputs,train_fea_inputs,train_labels,train_alloy_poss) in enumerate(train_loader):
            ...
            with autocast():
                train_outputs = model(train_process_inputs,train_fea_inputs,train_alloy_poss)
        ...
        model.float()
        model.eval()
        for step, (val_process_inputs,val_fea_inputs,val_labels,val_alloy_poss) in enumerate(val_loader):
            ...
            with torch.no_grad():
                val_outputs = model(val_process_inputs,val_fea_inputs,val_alloy_poss)
...
def test_loop(model,test_loader):
    ...
    model.to(device)
    model.eval()
    for step, (test_process_inputs,test_fea_inputs,test_labels,test_alloy_poss) in enumerate(test_loader):
        ...
        with torch.no_grad():
            test_outputs = model(test_process_inputs,test_fea_inputs,test_alloy_poss)
...
if __name__ == "__main__":
    ...
    train_dataset = PEDataset(train_process_texts,train_fea_texts,train_labels,train_alloy_poss)
    ...
    val_dataset = PEDataset(val_process_texts,val_fea_texts,val_labels,val_alloy_poss)
    val_loader = data_loader(val_dataset, 128, False)
    #模型训练
    model_name_pyh = "task1-v3/contras_learning/encoder_x/phy_encoder" 
    model_name_pro = "task1-v3/contras_learning/encoder_x/pro_encoder"
    model = Model(model_name_pyh,model_name_pro)
    epoch = 120
    lr = 1e-4
    ...
    df_val.to_excel('task1-v3/contras_learning/results/val/'+str(time)+".xlsx",  index=False)
    df_train = pd.DataFrame({'Train Outputs': train_outputs, 'Train Labels': train_labels})
    df_train.to_excel('task1-v3/contras_learning/results/train/'+str(time)+".xlsx",  index=False)
    trained_model.float()
    #测试集数据加载
    ...
    test_dataset = PEDataset(test_process_texts, test_fea_texts, test_labels, test_alloy_poss)
    ...
    df.to_excel('task1-v3/contras_learning/results/test/'+str(time)+".xlsx",  index=False)